      ...
      <div style={starContainerStyle}>
        {Array.from({ length: maxRating }, (_, i) =>
          < Star key={i}
            ...
            color={color}
            size={size}
            isFull={!temporaryRating ? rating >= i + 1 : temporaryRating >= i + 1} />)}
      </div>
      <p style={textStyle}>{messages.length !== maxRating ?
        (temporaryRating || rating || '') :
        (messages[temporaryRating ? temporaryRating - 1 : rating - 1])}</p>
    </div>
  );
}


function Star({ onStarClick, onMouseEnter, onMouseLeave, color, size, isFull = false }) {
  ...
  return (
    <span onClick={onStarClick}
      onMouseDownCapture={onStarClick}
